...
#define MAX_CHUNKS_IN_FLIGHT 1

static struct blecon_event_loop_t* _event_loop = NULL;
static struct blecon_t* _blecon = NULL;
...
void ota_dfu_thread_fn(void*, void*, void*) {
    while(true) {
        ...
        // Signal event loop
        blecon_event_signal(_process_completed_dfu_target_events_event);
    }
}
